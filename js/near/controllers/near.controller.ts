...
class NearController{
    ...
    async deployContractWithCustomMetadata(req: express.Request, res: express.Response) {
        const account = await NearService.deployContractWithCustomMetadata(req.body);
        res.status(201).send({account});
    }
